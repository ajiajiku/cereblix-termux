...
/* IDs of submit requests awaiting a pool response.  Login and keepalive
 * responses must never be counted as accepted/rejected shares. */
...
static void handle_line(char *l) {
    ...
    if (strstr(l, "\"job\"")) {
        ...
        /* Some pools send the initial job/login information separately. */
        char sid[128];
        if (json_str(res, "id", sid, sizeof sid))
            snprintf(session, sizeof session, "%s", sid);
    }
